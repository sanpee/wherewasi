...
class query:
  ...
  def __str__(self):
    ...
    return '{0} or "{1}", {2} within {3}km'.format(
      self.position, self.location_name, 
      timeText,
      self.distance) 
  ...
  def reform(self) -> str:
    ...
    return '{0} {1} within {2}km'.format(
      f'({self.location_name})' if len(self.location_name) > 0 else self.position, 
      timeText,
      self.distance) 
