...
class result:
  def __init__(self, t = time(0,0), p = (0, 0), d = 0.0, a = {'Path', 'End', 'Start'}, id = 0) -> None:
    self.time = t
    self.position = p 
    self.distance = d
    self.activity = a
...
class googletimeline:
  ...
  def findLocation(self, targetLocation, distance_from_target_km, findTimeStart = None, findTimeEnd = None, include_raw_path = True ) -> list[result]:
    ...
    if include_raw_path:
      ...
      for t in timelinePaths:
        for pt in t['timelinePath']:
          ...
          if distFromTarget <= distance_from_target_km and googletimeline.isTimeInRange(findTimeStart, findTimeEnd, visitTime):
            # Tag the path
            if 'id' not in t.keys(): self.idcounter += 1; t['id'] = self.idcounter
